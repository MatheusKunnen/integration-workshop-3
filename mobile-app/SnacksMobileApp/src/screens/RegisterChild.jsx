...
function RegisterChild({ navigation}) {
    ...
    const registerChild = async () => {
        ...
        RegisterChildService.registerChild(child, token).then((response) => {
    ...
    const getImageSet = async () => {
        PasswordGroupService.getRandomPasswordGroup().then((response) => {
            console.log(response);
      ...
      if (response && typeof response === 'object') {
        ...
        if (id && typeof id === 'number') {
          ...
          console.error('Response is missing or has an invalid id:', response);
